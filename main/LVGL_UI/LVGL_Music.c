...
const lv_font_t * font_small=&ui_font_chinese12;
const lv_font_t * font_large=&ui_font_chinese16;
static bool Playing_Flag;                                     
uint32_t track_id = -1;//note:原本是0
...
void Start_animation() {
    ...
    lv_anim_set_values(&Music_img_animation, Music_img_angle, 3600 + Music_img_angle); 
...
lv_obj_t * create_ctrl_box(lv_obj_t * parent)
{
  lv_obj_t * cont = lv_obj_create(parent);
  lv_obj_remove_style_all(cont);                                                                 
  lv_obj_set_height(cont, LV_SIZE_CONTENT);//note:原本没有下面这行
  // lv_obj_set_size(cont, 300, LV_SIZE_CONTENT);                                                       
  lv_obj_set_style_pad_bottom(cont, 8, 0);
  //note:原本下面是1，4，4，5，4，4，1                                                      
  static const lv_coord_t grid_col[] = {LV_GRID_FR(1), LV_GRID_FR(3), LV_GRID_FR(3), LV_GRID_FR(4), LV_GRID_FR(3), LV_GRID_FR(3), LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
...
void track_load(uint32_t id) {
    if (first_Flag && id == track_id) return;

    time_act = 0;
    bool next = (track_id + 1) % ACTIVE_TRACK_CNT == id;
    ...
    lv_anim_set_time(&a, 500);
    lv_anim_set_path_cb(&a, lv_anim_path_ease_out);
    lv_anim_set_values(&a, 0, next ? -LV_HOR_RES/2 : LV_HOR_RES/2);
    lv_anim_set_exec_cb(&a, _obj_set_x_anim_cb);
    lv_anim_set_ready_cb(&a, lv_obj_del_anim_ready_cb);
    lv_anim_start(&a);
...
lv_obj_t * create_List_box(lv_obj_t * parent)
{
  ...
  // _lv_demo_music_list_btn_check(track_id, true);//note:原本没有注释                                         
  return list;
}
lv_obj_t * add_list_btn(lv_obj_t * parent, uint32_t List_id)
{
  ...
  lv_obj_add_event_cb(btn,ui_event_Button1,LV_EVENT_CLICKED,NULL);//note：此处为我新添加的代码
  ...
  ESP_LOGI("add_list_btn","%s",File_Name[List_id]);                       
...
void _lv_demo_music_play(uint32_t id)
{
  if(Playing_Flag && id == track_id){                             
    ...
    {                  
      ...
      LVGL_Play_Music(id);                                        
      track_load(id);  //note:原本和上面的位置发生了交换                                                                       
      _lv_demo_music_resume();                               
    }
  }
}
...
{        
  ACTIVE_TRACK_CNT = Folder_retrieval("/sdcard",".mp3",SD_Name,100);//遍历MP3文件
  if(ACTIVE_TRACK_CNT) 
  {  
    for (int i = 0; i < ACTIVE_TRACK_CNT; i++) {
      strcpy(File_Name[i], SD_Name[i]);
      remove_file_extension(File_Name[i]);//移除文件后缀 
    }
    // if(track_id==0)//note:原本没有这行   
    // LVGL_Play_Music(0);    
  }                                                             
}
